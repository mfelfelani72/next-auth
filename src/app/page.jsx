...
// NOTE: No external libraries used. Pure React + CSS (Tailwind classes kept for convenience,
// but all interactive behaviour and animations are implemented with plain JS/CSS).
const mockData = {
    name: "Ali Reza",
    ...
    projects: [
        ...
        },
        {
            ...
            description: "High-throughput messaging with optimistic UI, message batching and media attachments.",
...
function Nav({ active, onToggleMobile, }) {
    const items = [
        { id: "home", label: "Home" },
    ...
    return (<header className="fixed inset-x-0 top-4 z-40 px-4">
      <div className="max-w-5xl mx-auto bg-white/95 backdrop-blur-sm rounded-2xl p-3 flex items-center justify-between shadow-sm border">
        ...
        <nav className="hidden md:flex items-center gap-2">
          {items.map((it) => (<a key={it.id} href={`#${it.id}`} className={`px-3 py-1 rounded-md text-xs font-medium transition transform ${active === it.id
                ? "bg-indigo-600 text-white scale-105 shadow"
                : "text-gray-700 hover:scale-105"}`}>
              {it.label}
            </a>))}
          <a href={`mailto:${mockData.contact.email}`} className="ml-3 px-3 py-1 rounded-md bg-amber-400 text-white text-xs font-semibold">
            Hire
          </a>
        </nav>

        <div className="md:hidden">
          <button onClick={onToggleMobile} aria-label="Toggle menu" className="p-2 rounded-md bg-white shadow-sm">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path d="M4 7h16M4 12h16M4 17h16" stroke="#111827" strokeWidth="1.6" strokeLinecap="round"/>
            </svg>
          </button>
        </div>
      </div>
...
function MobileDrawer({ open, onClose, active, }) {
    ...
    if (!open)
        return null;
    return (<div className="max-w-5xl mx-auto mt-2 bg-white rounded-xl p-3 shadow-sm">
      <div className="flex flex-col gap-2">
        {items.map((id) => (<a key={id} href={`#${id}`} onClick={onClose} className={`px-3 py-2 rounded-md text-sm ${active === id
...
function SkillProgress({ skill }) {
    return (<div className="mb-3">
      ...
      <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
        <div className={`h-2 rounded-full ${skill.colorClass}`} style={{ width: `${skill.level}%`, transition: "width 900ms ease" }}/>
      </div>
    </div>);
}
function ProjectGallery({ images }) {
    ...
    return (<div className="relative w-full rounded-md overflow-hidden">
      <img src={images[index]} alt={`project-${index}`} className="w-full h-44 object-cover transition-transform duration-700"/>
      <div className="absolute right-2 bottom-2 flex gap-2">
        {images.map((_, i) => (<button key={i} onClick={() => setIndex(i)} className={`w-2 h-2 rounded-full ${i === index ? "bg-white" : "bg-white/40"}`} aria-label={`Go to image ${i + 1}`}/>))}
      </div>
    </div>);
}
export default function RootPage() {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-white to-slate-50 text-slate-900 text-sm">
      ...
      <div className="pt-28 px-4">
        ...
        <main className="max-w-5xl mx-auto pb-20">
          <section id="home" className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
            <div className="md:col-span-2 bg-white rounded-xl p-5 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <a href={mockData.contact.github} target="_blank" rel="noreferrer" className="text-xs px-3 py-1 rounded-md bg-sky-100 text-sky-700">
                  GitHub
                </a>
                <a href={mockData.contact.linkedin} target="_blank" rel="noreferrer" className="text-xs px-3 py-1 rounded-md bg-rose-100 text-rose-700">
                  LinkedIn
                </a>
              </div>
            ...
            <aside className="bg-white rounded-xl p-4 shadow-sm sticky top-28">
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="w-full mt-2">
                  {mockData.skills.slice(0, 4).map((s) => (<SkillProgress key={s.name} skill={s}/>))}
                </div>
                <a href={`mailto:${mockData.contact.email}`} className="w-full text-center px-3 py-1 rounded-md bg-indigo-600 text-white text-xs">
                  Contact
                </a>
              </div>
            </aside>
          ...
          <section id="skills" className="mt-8 bg-white rounded-xl p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
              <div>
                {mockData.skills.map((s) => (<SkillProgress key={s.name} skill={s}/>))}
              </div>
              <div>
                <h3 className="text-xs font-semibold">Tooling</h3>
                <div className="flex flex-wrap gap-2 mt-2">
                  {mockData.tools.map((t, i) => (<span key={i} className="text-xs px-2 py-1 bg-slate-100 rounded-md">
                      {t}
                    </span>))}
                </div>
                ...
                <ul className="list-disc list-inside text-gray-700 text-sm mt-2">
                  {mockData.networkingExperience.map((n) => (<li key={n.id}>
          ...
          <section id="projects" className="mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
              {mockData.projects.map((p) => (<article key={p.id} className="bg-white rounded-xl p-3 shadow-sm">
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {p.technologies.map((t, i) => (<span key={i} className="text-xs px-2 py-1 bg-slate-100 rounded-md">
                          {t}
                        </span>))}
                    </div>
                    <div className="mt-2 flex gap-2">
                      <a href={p.link} target="_blank" rel="noreferrer" className="text-xs px-2 py-1 rounded-md bg-indigo-600 text-white">
                        GitHub
                      </a>
                    </div>
                  </div>
          ...
          <section id="certs" className="mt-8 bg-white rounded-xl p-4 shadow-sm">
            ...
            <ul className="mt-2 text-sm text-gray-700 list-disc list-inside">
              {mockData.certifications.map((c) => (<li key={c.id}>
                  {c.title} — {c.year}
                </li>))}
            </ul>
          </section>
          ...
          <section id="languages" className="mt-8 bg-white rounded-xl p-4 shadow-sm">
            ...
            <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3">
              {mockData.languages.map((l) => (<div key={l.name} className="p-3 bg-slate-50 rounded-md text-sm">
          ...
          <section id="contact" className="mt-8 bg-white rounded-xl p-4 shadow-sm text-center">
            <h2 className="text-sm font-semibold text-indigo-700">Contact</h2>
            <p className="text-xs text-gray-700 mt-2">
              Open to new opportunities — hire or contract.
            </p>
            <div className="mt-3 flex flex-wrap justify-center gap-2">
              <a href={`mailto:${mockData.contact.email}`} className="px-3 py-1 rounded-md bg-indigo-600 text-white text-xs">
                Email
              </a>
              <a href={mockData.contact.linkedin} target="_blank" rel="noreferrer" className="px-3 py-1 rounded-md bg-rose-100 text-rose-700 text-xs">
                LinkedIn
              </a>
              <a href={mockData.contact.github} target="_blank" rel="noreferrer" className="px-3 py-1 rounded-md bg-sky-100 text-sky-700 text-xs">
                GitHub
              </a>
            </div>
          </section>
      ...
      <a href={`mailto:${mockData.contact.email}`} className="fixed right-4 bottom-4 z-40 rounded-full bg-gradient-to-r from-pink-500 to-yellow-400 text-white px-4 py-2 text-sm font-semibold shadow-lg">
        Hire
      </a>

      {/* Small inline CSS for gentle animations and gradients (no external libs) */}
      <style>{`
